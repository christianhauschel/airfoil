...
References
----------
https://raw.githubusercontent.com/mdolab/prefoil/c59f087134517f2d4a9f1a621284da1f3abf7a67/prefoil/sampling.py
"""
...
def sampling(spacing, start, end, n, **kwargs):
    if spacing == "cosine":
        fct_sampling = cosine
    elif spacing == "conical":
        fct_sampling = conical
    elif spacing == "polynomial":
        fct_sampling = polynomial
    else:
        fct_sampling = bigeometric
    return fct_sampling(start, end, n, **kwargs)
...
def cosine(start, end, n, m=np.pi):
    ...
    """
    return conical(start, end, n, m, coeff=1)
...
def conical(start, end, n, m=np.pi, coeff=1, bad_edge=False):
    ...
    if b >= 1:
        s = (
            1 + 1 / np.sqrt(np.cos(x) ** 2 + np.sin(x) ** 2 / b**2) * np.cos(x)
        ) * 0.5
    else:
        cos = np.cos(x)
...
def bigeometric(start, end, n, a1=0.001, b1=0.001, ra=1.1, rb=1.1):
    ...
    if checkleft < 0 and checkright < 0:
        ...
        na = optimize.bisect(
            findSpacing, left, right, xtol=1e-4, maxiter=100, disp=False
        )
...
def joined_spacing(n, spacingFunc=polynomial, func_args=None, s_LE=0.5):
    """
